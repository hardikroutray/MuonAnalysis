...
for i, event in enumerate(tree_mu):
    ...
    mu4p = TLorentzVector(0, 0, 0, 0)
    mu1p_uncorr = TLorentzVector(0, 0, 0, 0)
    mu2p_uncorr = TLorentzVector(0, 0, 0, 0)
    mu3p_uncorr = TLorentzVector(0, 0, 0, 0)
    mu4p_uncorr = TLorentzVector(0, 0, 0, 0)
...
#    print "In event", i , "passL1?", passL1, "passHLT?", passHLT
    ...
    num_muons[0] = len(event.muon_pt)

    for m, muon in enumerate(len(event.muon_pt)):

        muon_pt[m] = event.muon_pt[m]
        muon_eta[m] = event.muon_pt[m]
        muon_phi[m] = event.muon_phi[m]
        muon_q[m] = event.muon_q[m]
